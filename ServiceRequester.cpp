...
namespace RTT
{
    ...
    bool ServiceRequester::addOperationCaller(OperationCallerBaseInvoker& isb)
    {
        ...
        {
          cout << "OperationCaller with name '" + isb.getName() + "' already present." << endl;
            return false;
        }
    ...
    bool ServiceRequester::connectTo( Service::shared_ptr sp) {
        for (OperationCallers::iterator it = mmethods.begin(); it != mmethods.end(); ++it) {
            if ( !it->second->ready() ) {
                if (sp->hasOperation( it->first )) {
                    ...
                    if ( it->second->ready() ) {
                        if (mrowner)
                            cout << "Successfully set up OperationCaller " << it->first <<endl;
                        else
                          cout << "OperationCaller "<< it->first << " has no caller set."<<endl;
                    }
                }
        ...
        if (ready()) {
            if (!mprovider)
                mprovider = sp;
            cout << "Found complete interface of requested service '" << mrname <<"'"<< endl;
            return true;
        }
    ...
    bool ServiceRequester::ready() const
    {
        for (OperationCallers::const_iterator it = mmethods.begin(); it != mmethods.end(); ++it)
            if ( !it->second->ready() ) {
              cout << "ServiceRequeste: "<< it->first << " not set up." <<endl;
                return false;
            }
        return true;
    }
