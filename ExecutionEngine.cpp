        ...
        if ( taskc ) {
            ...
            if ( taskc->mTaskState == TaskCore::Running && taskc->mTargetState == TaskCore::Running ) {
                try {
                    ...
                    cout << "in updateHook(): switching to exception state because of unhandled exception" << endl;
                    cout << "  " << e.what() << endl;
                    taskc->exception();
                } catch(...){
                    cout << "in updateHook(): switching to exception state because of unhandled exception" << endl;
            ...
            if (  taskc->mTaskState == TaskCore::RunTimeError ) {
                try {
                    ...
                    cout << "in errorHook(): switching to exception state because of unhandled exception" << endl;
                    cout << "  " << e.what() << endl;
                    taskc->exception();
                } catch(...){
                    cout << "in errorHook(): switching to exception state because of unhandled exception" << endl;
