...
namespace RTT
{
    ...
    public:
        ...
        {
            if ( source && ! _value ) {
                cout <<"Can not initialize Property from "<<source->getName() <<": ";
                if ( source->getDataSource() )
                  cout << "incompatible type ( destination type: "<< getType() << ", source type: "<< source->getDataSource()->getTypeName() << ")."<<endl;
                else
                    cout << "source Property was not ready."<<endl;
            }
        }
...
namespace RTT
{
    ...
    template<>
    void Property<PropertyBag>::identify( base::PropertyBagVisitor* pbi);
}
