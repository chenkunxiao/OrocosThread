...
#define OROSEM_OS_PERIODIC_THREADS_MAX_OVERRUN 10
...
namespace RTT {
    namespace os
    {
        ...
        void *thread_function(void* t)
        {
            ...
            while (!task->prepareForExit)
            {
                TRY
                {
                    ...
                    {
                        task->emergencyStop();
                        cout << rtos_task_get_name(task->getTask())
                                << " got too many periodic overruns in step() ("
                                << overruns << " times), stopped Thread !"
                                << endl;
                        cout   << " See Thread::setMaxOverrun() for info."
                                << endl;
                    }
        ...
        void Thread::setup(int _priority, unsigned cpu_affinity, const std::string& name)
        {
            ...
            /*  cout << "Creating Thread for scheduler=" << (msched_type == ORO_SCHED_OTHER ? "ORO_SCHED_OTHER" : "ORO_SCHED_RT")
                      << ", priority=" << _priority
            ...
            if (ret != 0)
            {
                cout << "Could not allocate configuration semaphore 'sem' for "
                     << name << ". Throwing std::bad_alloc." << endl;
...

            {
                ...
                {
                    cout << "Failed to find 'ThreadScope' object in DigitalOutInterface::nameserver." << endl;
                }
            }
#endif
        ...
        Thread::~Thread()
        {
            ...
            cout << "Terminating " << this->getName() << endl;
            terminate();
            cout << " done" << endl;
            rtos_sem_destroy(&sem);
        ...
        bool Thread::start()
        {
            if ( period == 0)
            {
                ...
                if (ret != 0)
                    cout << "Thread::start(): sem_signal returns " << ret << endl;
        ...
        bool Thread::stop()
        {
            ...
            if ( period == 0)
            {
                if ( inloop ) {
                    if ( !this->breakLoop() ) {
                        cout << "Failed to stop thread " << this->getName() << ": breakLoop() returned false."<<endl;
                        running = true;
                        return false;
                    }
                ...
                if ( !lock.isSuccessful() ) {
                    cout << "Failed to stop thread " << this->getName() << ": breakLoop() returned true, but loop() function did not return after 1 second."<<endl;
                    running = true;
                    return false;
                }
            } else {
                ...
                if ( lock.isSuccessful() ) {
                    ...
                    cout << "Failed to stop thread " << this->getName() << ": step() function did not return after "<< 5*getPeriod() <<" seconds."<<endl;
                    running = true;
                    return false;
                }
            }
        ...
        bool Thread::setScheduler(int sched_type)
        {
            ...
            cout << "Setting scheduler type for Thread '"
