...
{ namespace os {
	...
	if ( cpu_affinity != ~0 ) {
	  cout << "Setting CPU affinity to " << cpu_affinity << endl;
	  if (0 != rtos_task_set_cpu_affinity(task, cpu_affinity))
	    {
	      cout << "Failed to set CPU affinity to " << cpu_affinity << endl;
	    }
	}
  ...
  INTERNAL_QUAL void rtos_task_set_wait_period_policy( RTOS_TASK* task, int policy )
  {
    ...
    INTERNAL_QUAL int rtos_task_check_scheduler(int* scheduler)
    {
        ...
        if (*scheduler != SCHED_OTHER && geteuid() != 0
            ...
            {
                cout << "Lowering scheduler type to SCHED_OTHER for non-privileged users.." <<endl;
                *scheduler = SCHED_OTHER;
                return -1;
            }
        }
        ...
        if (*scheduler == SCHED_OTHER) {
            if ( *priority != 0 ) {
                if (*priority != LowestPriority)
                    cout << "Forcing priority ("<<*priority<<") of thread with SCHED_OTHER policy to 0." <<endl;
                *priority = 0;
                ret = -1;
            }
        } else {
            // SCHED_FIFO/SCHED_RR:
            if (*priority <= 0){
                cout << "Forcing priority ("<<*priority<<") of thread with !SCHED_OTHER policy to 1." <<endl;
                *priority = 1;
                ret = -1;
            }
            if (*priority > 99){
                cout << "Forcing priority ("<<*priority<<") of thread with !SCHED_OTHER policy to 99." <<endl;
                *priority = 99;
                ret = -1;
            }
                 ...
                 )
            {
                ...
                if (0 == getrlimit(RLIMIT_RTPRIO, &r))
                {
                    ...
                    {
                        cout << "Forcing priority ("<<*priority<<") of thread with !SCHED_OTHER policy to the pam_limit of " << r.rlim_cur <<endl;
                        *priority = r.rlim_cur;
                        ret = -1;
                    }
                }
