...
namespace RTT {
                ...
                if ( sz == args.size() ) {
                    ...
                    if (rta)
                        try {
                          ...
                          cout << "Error in OperationCallerC::ret : can not convert return value of type "<< m->getType() << " to given type "<< rta->getType()<<endl;
                        }
        ...
        : d( mr ? new D( mr, name, caller) : 0 ), m(), ofp(mr), mname(name)
    {
        if ( d && d->m ) {
            ...
            if (mr == 0)
                cout <<"Can not construct OperationCallerC for '"<<name<<"' from null OperationInterfacePart."<<endl;
        }
    }
    ...
    OperationCallerC& OperationCallerC::arg( DataSourceBase::shared_ptr a )
    {
        if (d)
            d->newarg( a );
             ...
             cout << "Extra argument discarded for OperationCallerC."<< endl;
        }
        if ( d && d->m ) {
            this->m = d->m;
            this->s = d->s;
        ...
        if (d)
            ...
            if (m) {
                try {
                  ...
                  cout << "Error in OperationCallerC::ret : can not convert return value of type "<< m->getType() << " to given type "<< r->getDataSource()->getType()<<endl;
                }
            } else
              cout <<"Can not add return argument to invalid OperationCallerC."<<endl;
        }
        return *this;
    }
    ...
    bool OperationCallerC::call() {
        if (m)
          ...
          cout << "call() called on incomplete OperationCallerC."<< endl;
            if (d) {
                size_t sz;
                sz = d->ofp->arity();
                cout << "Wrong number of arguments provided for method '"+d->mname+"'"<< endl;
                cout << "Expected "<< sz << ", got: " << d->args.size() << endl;
            }
        }
        return false;
    }
    ...
    SendHandleC OperationCallerC::send() {
        ...
        try {
          ...
          cout <<"OperationCallerC::send(): Can not send the '" << ofp->getName() << "' operation:" << nao.what()  << endl;
            return SendHandleC();
        }
        if (s) {
          ...
          cout << "send() called on incomplete OperationCallerC."<< endl;
            if (d) {
                size_t sz;
                sz = d->ofp->arity();
                cout << "Wrong number of arguments provided for method '"+d->mname+"'"<< endl;
