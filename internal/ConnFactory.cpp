...
bool ConnFactory::createAndCheckConnection(base::OutputPortInterface& output_port, base::InputPortInterface& input_port, base::ChannelElementBase::shared_ptr channel_input, ConnPolicy policy) {
    ...
    if ( output_port.addConnection( input_port.getPortID(), channel_input, policy ) ) {
        ...
        if ( input_port.channelReady( channel_input->getOutputEndPoint() ) == false ) {
            ...
            cout << "The input port "<< input_port.getName()
                       << " could not successfully read from the connection from output port " << output_port.getName() <<endl;

            return false;
        }
        cout << "Connected output port "<< output_port.getName()
                  << " successfully to " << input_port.getName() <<endl;
        return true;
    }
    // setup failed.
    channel_input->disconnect(true);
    cout << "The output port "<< output_port.getName()
               << " could not successfully use the connection to input port " << input_port.getName() <<endl;
    return false;
}
/*
                                                                                     ...
                                                                                     StreamConnID* conn_id) 
{
    ...
    if (ttt) {
        ...
        cout <<"Could not determine sample size for type " << type->getTypeName() << endl;
    }
    ...
    if ( input_port.isLocal() ) {
        ...
        if (ceb_input) {
            cout <<"Receiving data for port "<<input_port.getName() << " from out-of-band protocol "<< policy.transport << " with id "<< policy2.name_id<<endl;
        } else {
            cout << "The type transporter for type "<<type->getTypeName()<< " failed to create a remote channel for port " << input_port.getName()<<endl;
            return 0;
        }
    ...
    if ( output_port.isLocal() ) {
        ...
        if (ceb_output) {
            cout <<"Redirecting data for port "<< output_port.getName() << " to out-of-band protocol "<< policy.transport << " with id "<< policy2.name_id <<endl;
        } else {
            cout << "The type transporter for type "<<type->getTypeName()<< " failed to create a remote channel for port " << output_port.getName()<<endl;
            return 0;
        }
