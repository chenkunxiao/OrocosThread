...
namespace RTT {
          ...
          //  Logger::In in("SendHandleC");
            if ( mofp ) {
                ...
                if ( sz == args.size() ) {
                    ...
                    args.clear();
                    if ( !s ) {
                      cout << "Failed to produce collector for "<< mname << " with " << sz << " arguments." << endl;
                        return;
                    }
                }
            }
    ...
    SendHandleC& SendHandleC::arg( DataSourceBase::shared_ptr a )
    {
        if (d)
          ...
          cout << "Extra argument discarded for SendHandleC."<< endl;
        }
        if ( d && d->s ) {
            e->s = d->s;
            e->b = d->blocking;
    ...
    SendStatus SendHandleC::collect() {
        if (e->s) {
            ...
            cout << "collect() called on incomplete SendHandleC."<<endl;
            if (d) {
                size_t sz;
    ...
    SendStatus SendHandleC::collectIfDone() {
        if (e->s) {
          ...
          cout << "collectIfDone() called on incomplete SendHandleC."<< endl;
            if (d) {
                size_t sz;
                sz = d->mofp->collectArity();
                cout << "Wrong number of arguments provided for method '"+d->mname+"'"<< endl;
