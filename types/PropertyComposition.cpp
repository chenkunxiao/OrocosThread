...
{
    if ( !target.empty() ) {
      cout <<"composePropertyBag: target bag must be empty."<< endl;
        return false;
    }
    ...
    for( ; sit != sourcebag.end(); ++sit) {
        ...
        if ( isbag.ready() && isbag.value().getType() != "PropertyBag") {
            ...
            if ( ti == 0) {
                cout <<"Could not compose unknown type '" << isbag.value().getType() <<"'." <<endl;
                has_error = true;
                continue;
            }
            ...
            if ( ti->composeType(isbag.getDataSource(), tgtprop->getDataSource()) ) {
                cout << "Used user's composition function for " << tgtprop->getName() <<":"<<tgtprop->getType()<<endl;
                target.ownProperty(tgtprop);
            } else {
                cout <<"The type '" << isbag.value().getType() <<"' did not provide a type composition function, but I need one to compose it from a PropertyBag." <<endl;
                delete tgtprop;
                has_error = true;
                continue;
            }
...
{
    if ( !target.empty() ) {
      cout <<"decomposePropertyBag: target bag must be empty."<< endl;
        return false;
    }
    PropertyBag::const_iterator sit = sourcebag.begin();
    while( sit != sourcebag.end()) {
        ...
        if ( isbag.ready() ) {
          ...
          cout << "Checking for decompose "<< (*sit)->getName() <<endl;
                ...
                if ( dsb == (*sit)->getDataSource() ) {
                    ...
                    if ( bagds ) {
                        ...
                        if ( target.ownProperty( p ) == false)
                           cout <<"Failed to create a property of decomposed data of type "<<(*sit)->getType() <<endl;
                    }
                }
            } else {
