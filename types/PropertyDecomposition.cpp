...
bool typeDecomposition( base::DataSourceBase::shared_ptr dsb, PropertyBag& targetbag, bool recurse)
{
    ...
    if ( parts.empty() ) {
        cout << "propertyDecomposition: "<<  dsb->getTypeName() << " does not have any members." << endl;
        return false;
    }
    ...
    for(vector<string>::iterator it = parts.begin(); it != parts.end(); ++it ) {
        ...
        if (!part) {
            cout <<"propertyDecomposition: Inconsistent type info for "<< dsb->getTypeName() << ": reported to have part '"<<*it<<"' but failed to return it."<<endl;
            continue;
        }
        if ( !part->isAssignable() ) {
            // For example: the case for size() and capacity() in SequenceTypeInfo
            cout <<"propertyDecomposition: Part "<< *it << ":"<< part->getTypeName() << " is not changeable."<<endl;
            continue;
        }
        // finally recurse or add it to the target bag:
        PropertyBase* newpb = part->getTypeInfo()->buildProperty(*it,"Part",part);
        if ( !newpb ) {
            cout << "Decomposition failed because Part '"<<*it<<"' is not known to type system."<<endl;
            continue;
        }
        ...
        for (int i=0; i < msize; ++i) {
            ...
            if (item) {
                if ( !item->isAssignable() ) {
                    // For example: the case for size() and capacity() in SequenceTypeInfo
                  cout <<"propertyDecomposition: Item '"<< indx << "' of type "<< dsb->getTypeName() << " is not changeable."<<endl;
                    continue;
                }
                // finally recurse or add it to the target bag:
    ...
    if (targetbag.empty() )
      cout << "propertyDecomposition: "<<  dsb->getTypeName() << " returns an empty property bag." << endl;
    return true;
}
