...
namespace RTT
{
    ...
    bool storeProperty(PropertyBag& bag, const std::string& path, base::PropertyBase* item, const std::string& separator )
    {
      //   Logger::In in("storeProperty");
        ...
        if ( !parentbag ) {
          cout << "Path component '" << pname << "' in path '"<<path<<"' does not point to a PropertyBag."<<endl;
            return false;
        }
...
#ifndef NDEBUG
        if (update)
            cout << "updateProperties: updating Property "
            << source->getType() << " "<< source->getName()
                 << "." << endl;
        else
            cout << "refreshProperties: refreshing Property "
            << source->getType() << " "<< source->getName()
                 << "." << endl;
#endif
    ...
    bool refreshProperties(const PropertyBag& target, const PropertyBag& source, bool allprops)
    {
      //        Logger::In in("refreshProperties");
        ...
        if ( Types()->type(target.getType()) != Types()->getTypeInfo<PropertyBag>() && Types()->type( target.getType() ) != Types()->type( source.getType() ) ) {
          cout << "Can not populate typed PropertyBag '"<< target.getType() <<"' from '"<<source.getType()<<"' (source and target type differed)."<<endl;
            return false;
        }
                ...
                if ( mit != mines.end() ) {
                ...
                else
                    {
                      ...
                      //             Logger::log() << Logger::Debug;
                        cout << "updateProperties: creating Property "
        ...
        if (source_walker != 0 )
        {
            ...
            if ( source_walker_bag != 0 && start != std::string::npos ) {
                if ( target_walker_bag == 0 ) {
                    cout << "Property '"<<target_walker->getName()<<"' is not a PropertyBag !"<<endl;
                    return false;
                }
                ...
                if (updateOrRefreshProperty( source_walker, target_walker, true) == false)
                    return false;
                cout << "Found Property '"<<target_walker->getName() <<"': update done." << endl;
                return true;
            }
        } else {
            // error wrong path, not present in source !
            cout << "Property '"<< token <<"' is not present in the source PropertyBag !"<<endl;
            return false;
        }
        // not reached.
    ...
    bool refreshProperty(PropertyBag& target, const PropertyBag& source, const std::string& name, const std::string& separator)
    {
      //Logger::In in("refreshProperty");
        ...
        {
            if ( target_walker == 0 ) {
              cout << "Property '"<<source_walker->getName()<<"' was not found in target !"<<endl;
                return false;
            }
            Property<PropertyBag>*  source_walker_bag;
            Property<PropertyBag>*  target_walker_bag;
            ...
            if ( source_walker_bag != 0 && start != std::string::npos ) {
                ...
                if (updateOrRefreshProperty( source_walker, target_walker, false) == false)
                    return false;
                cout << "Found Property '"<<target_walker->getName() <<"': refresh done." << endl;
                return true;
            }
        } else {
            // error wrong path, not present in source !
