...
namespace RTT {
    ...
    bool Service::addService( Service::shared_ptr obj ) {
        if ( services.find( obj->getName() ) != services.end() ) {
          cout << "Could not add Service " << obj->getName() <<": name already in use." <<endl;
            return false;
        }
    ...
    Service::shared_ptr Service::provides() {
        try {
          ...
          cout <<"When using boost < 1.40.0 : You are not allowed to call provides() on a Service that does not yet belong to a TaskContext or another Service." << endl;
          cout <<"Try to avoid using provides() in this case: omit it or use the service directly." <<endl;
          cout <<"OR: upgrade to boost 1.40.0, then this error will go away." <<endl;
    ...
    OperationInterfacePart* Service::getOperation( std::string name )
    {
           ...
           cout << "No such operation in service '"<< getName() <<"': "<< name <<endl;
        return 0;
    }
    ...
    bool Service::addLocalOperation( OperationBase& op )
    {
        ...
        if ( op.getName().empty() ) {
            cout << "Failed to add Operation: '"<< op.getName() <<"' has no name." <<endl;
            return false;
        }
        // don't check ready() since the op may not have an owner yet:
        if ( !op.getImplementation() ) {
          cout << "Failed to add Operation: '"<< op.getName() <<"' is not ready: not bound to a function." <<endl;
            return false;
        }
        if ( simpleoperations.count( op.getName() ) ) {
            cout << "While adding Operation: '"<< op.getName() <<"': replacing previously added operation." <<endl;
