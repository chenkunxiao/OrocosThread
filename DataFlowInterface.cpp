...
namespace RTT
{
    ...
    PortInterface& DataFlowInterface::addPort(PortInterface& port) {
        ...
        if (mservice && mservice->hasService( port.getName()) != 0) {
            cout <<"'addPort' "<< port.getName() << ": name already in use as Service. Replacing previous service with new one." <<endl;
            mservice->removeService(port.getName());
        }

        if (!mservice) {
            cout <<"'addPort' "<< port.getName() << ": DataFlowInterface not given to parent. Not adding Service." <<endl;
            return port;
        }
    ...
    PortInterface& DataFlowInterface::addLocalPort(PortInterface& port) {
        for ( Ports::iterator it(mports.begin());
            ...
            if ( (*it)->getName() == port.getName() ) {
                cout <<"'addPort' "<< port.getName() << ": name already in use. Disconnecting and replacing previous port with new one." <<endl;
    ...
    InputPortInterface& DataFlowInterface::addLocalEventPort(InputPortInterface& port, SlotFunction callback) {
        ...
        if (mservice == 0 || mservice->getOwner() == 0) {
          cout << "addLocalEventPort "<< port.getName() <<": DataFlowInterface not part of a TaskContext. Will not trigger any TaskContext nor register callback." <<endl;
            return port;
        }
        ...
        if (h) {
          cout << mservice->getName() << " will be triggered when new data is available on InputPort " << port.getName() << endl;
            handles.push_back(h);
        } else {
            cout << mservice->getName() << " can't connect to event of InputPort " << port.getName() << endl;
            return port;
        }
#endif
        if (callback)
    ...
    {
        if ( ptr == 0) {
            cout << "You tried to add a null pointer in '"<< where << "' for the object '" << name << "'. Fix your code !"<< endl;
            return false;
        }
        return true;
    }
