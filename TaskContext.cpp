...
namespace RTT
{
    ...
    void TaskContext::setup()
    {
        ...
        //    this->addOperation("read",&InputPortInterface::read());
    ...
    bool TaskContext::connectPorts( TaskContext* peer )
    {
      ...
      //  Logger::In in( location.c_str()  );
        ...
        for (DataFlowInterface::Ports::iterator it = myports.begin();
            ...
            if ( !peerport ) {
              cout << "Peer Task "<<peer->getName() <<" has no Port " << (*it)->getName() << endl;
                continue;
            }
            ...
            if((dynamic_cast<OutputPortInterface*>(*it) && dynamic_cast<OutputPortInterface*>(peerport)) ||
              ...
              {
                cout << (*it)->getName() << " and " << peerport->getName() << " have the same type" << endl;
                continue;
              }

            // Try to find a way to connect them
            if ( !(*it)->connectTo( peerport ) ) {
                cout << "Data flow incompatible between ports "
                          << getName() << "." << (*it)->getName() << " and "
                          << peer->getName() << "." << (*it)->getName() << endl;
                failure = true;
            }
        }
        return !failure;
        ...
        for (vector<string>::iterator it = myreqs.begin();
            ...
            if ( !sr->ready() ) {
                if (peer->provides()->hasService( *it ))
                    ...
                    cout<< "Peer Task "<<peer->getName() <<" provides no Service " << *it << endl;
                }
            }
        }
        ...
        for (vector<string>::iterator it = peerreqs.begin();
            ...
            if ( !sr->ready() ) {
                if (this->provides()->hasService(*it))
                  ...
                  cout<< "This Task provides no Service " << *it << " for peer Task "<<peer->getName() <<"."<< endl;
            }
        }
        return !failure;
    }
